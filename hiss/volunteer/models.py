...
class Event(models.Model):
    hacker = models.ForeignKey('self', on_delete=models.SET_NULL, null=True)
...
class FoodEvent(Event):
    ...
    restrictions = models.CharField(max_length=14, choices=application_models.DIETARY_RESTRICTIONS)
    # hacker = models.CharField(max_length=14)
    # hacker = models.CharField("What school do you go to?", max_length=255)
    #hacker = models.ForeignKey('self', on_delete=models.SET_NULL, null=True)
...
class VolunteerApplication(models.Model):
    hacker = models.OneToOneField(
        'self', related_name="volunteer_app", on_delete=models.CASCADE
    )
    first_name = models.CharField(max_length=255)
    last_name = models.CharField(max_length=255)
    phone_number = PhoneNumberField()
    grad_year = models.CharField(
        "What is your anticipated graduation date?", choices=application_models.GRAD_YEARS, max_length=11
    )
    shirt_size = models.CharField(
        "Shirt size?", choices=application_models.SHIRT_SIZES, default=None, max_length=3
    )
